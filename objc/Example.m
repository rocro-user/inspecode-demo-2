...
char *STDCALL AStyleMain(const char *pSourceIn, const char *pOptions,
...
int main() {
    ...
    {   // options to pass to AStyle
        const NSArray *fileNames =
            [NSArray arrayWithObjects:@"AStyleDev/test-data/ASBeautifier.cpp",
                     @"AStyleDev/test-data/ASFormatter.cpp",
                     @"AStyleDev/test-data/astyle.h", nil];
        ...
        for (NSString *fileName in fileNames) { // get the text to format
            NSString *filePath = getProjectDirectory(fileName);
            NSString *textIn = getText(filePath);

            // call the Artistic Style formatting function
            char *textOutChar = AStyleMain([textIn UTF8String], [options UTF8String],
                                           ASErrorHandler, ASMemoryAlloc);
...
NSString *getProjectDirectory(const NSString *subPath) {
    ...
    NSString *projectPath = [NSString
                             stringWithFormat:@"%@/%@/%@", homeDirectory, @"Projects", subPath];
    return projectPath;
}
...
void setText(const NSString *textOut,
             NSString *filePath) { // create a backup file
    NSFileManager *fm = [NSFileManager defaultManager];
    NSString *origfilePath =
        [NSString stringWithFormat:@"%@%@", filePath, @".orig"];
